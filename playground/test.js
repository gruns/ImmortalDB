...
(async () => {
    ...
    $ele('get').addEventListener('click', async () => $value.value = await db.get($key.value), false);
    $ele('set').addEventListener('click', async () => await db.set($key.value, $value.value), false);
    $ele('remove').addEventListener('click', async () => {
        await db.remove($key.value);
        $value.value = '';
    }, false);

    $ele('removeCookie').addEventListener('click', () => Cookies.remove(PREFIX + $key.value));
    $ele('removeIndexedDB').addEventListener('click', async () => await idbKeyval.del(PREFIX + $key.value, idb));
    $ele('removeLocalStorage').addEventListener('click', () => localStorage.removeItem(PREFIX + $key.value));
    $ele('removeSessionStorage').addEventListener('click', () => sessionStorage.removeItem(PREFIX + $key.value));
